# PMA — Stari kolokviji i ispitni rokovi (13 zadataka)
...
## 2. kolokvij — Servis mobitela / POST varijanta (TIP A)
...
## 2. kolokvij — Prijava ispita (TIP A)
...
## 2. rok — Servis mobitela / GET+PUT varijanta
...
Kroz formu trebno je unijeti podatke o novom zaposleniku — ime i prezime, email adresu, godinu roditenja, kolegij i status (redovni/vanredni). Ime i prezime skupna ne smiju biti kraći od 6 znakova, email mora imat znak '@'. Svi podaci moraju biti uneseni. Redovni student mora biti rođen poslije godine 1990. (Gresku ispisite u `<p>` element sa id=„greska")

Prije slanja zahtjeva trebno je izračunati broj bodova koje student nosi za prijavu — osnovna plaća ovisi o poziciji (matematika: 50, informatika: 50, fizika: 30). Ako je kolegij Informatika, povećava se ukupan broj bodova za 5%. Ako student ima status redovnog studenta, povećava se ukupan broj bodova za 10 dodatnih bodova.

Nakon unosa svih podataka, pritiskom na tipku „Dodaj" trebno je poslati **POST zahtjev** na adresu http://<ip-adresa>:4000/studenti (ne smije se slati ako podaci nisu bili ispravni).
Format JSON podatka kojeg poslužitelj prihvaća možete vidjeti na slici (i poslužitelju)

Nakon slanja podataka, očistite podatke sa forme.

### Učitaj zapis

Kroz formu je trebno unijeti podatak za pretraživanje zaposlenika. Pritiskom na tipku „Pretraži" trebno je poslati **GET zahtjev** http://<ip-adresa>:4000/studenti sa parametrom „kolegiji" i „godinaRođenja".

Poslužitelj kao odgovor vraća niz odgovarajućih zaposlenika u JSON obliku (oblikovanje po želji, npr. numerirana lista sa svim detaljima studenta, primjer možete vidjeti na slici).

Nakon dohvaćanja podataka trebno ih je prikazati na sucelje, primjer možete vidjeti na slici (ne mora biti identičan).
...
Kroz formu trebno je unijeti podatke o novom sportašu — ime i prezime, email adresu, godinu roditenja, sport i status (profesionalan/amater). Ime i prezime skupna ne smiju biti kraći od 5 znakova, email mora imat znak '@'. Svi podaci trebaj biti uneseni. Profesionalni sportaš mora biti rodin poslije 1980. i prije 2005. (Gresku ispisite u `<p>` element sa id=„greska")

Prije slanja zahtjeva trebno je izračunati broj treninga tjedno — osnovna plaća ovisi o sportu (nogomет: 4, tenis: 3, plivanje: 5). Ako je sportaš profesionalan, povećava se broj treninga za 2. Ako je sport plivanje, povećava se broj treninga za 10%.

Nakon unosa svih podataka, pritiskom na tipku „Dodaj" trebno je poslati **POST zahtjev** na adresu http://<ip-adresa>:4000/sportasi (ne smije se slati ako podaci nisu bili ispravni).
Format JSON podatka kojeg poslužitelj prihvaća možete vidjeti na slici (i poslužitelju)
...
Kroz formu je trebno unijeti podatak za pretraživanje sportaša. Pritiskom na tipku „Pretraži" trebno je poslati **GET zahtjev** http://<ip-adresa>:4000/sportasi sa parametrom „sport" i „godinaRođenja".

Poslužitelj kao odgovor vraća niz odgovarajućih sportaša u JSON obliku (oblikovanje po želji, npr. numerirana lista sa svim detaljima sportaša, primjer možete vidjeti na slici).
...
- Server vraća:
  - Status 204 (No Content) ako zapis ne postoji → prikaži "Ne postoji podatak sa ovim ID-om"
